...
export default function PostCard({
  ...
  useEffect(() => {
    ;(async () => {
      const url = `${import.meta.env.VITE_BASE_SERVER_URL}/users/${authorId}`
      try {
        ...
        if (!jsonResponse.success) {
          console.log("Something went wrong while fetching the author data!")
          return
        }
                ...
                <div>
                  {!isAuthor ? (
                    ...
                    <p className="text-gray-900 font-semibold">
                      {authorDetails?.firstname} {authorDetails?.lastname}
                      <span className="text-red-600 text-sm ml-2">(You)</span>
                    </p>
                  )}
