  ...
  const submitFunction = async (_: IFormData, formData: FormData) => {
    ...
    try {
      if (!updatePage) {
        ...
        const url = `${baseUrl}/blogs/${updateBlogId}`
        const response = await fetch(url, {
          method: "PUT",
          credentials: "include",
          body: formData,
          ...
          >
            ...
            <div className="title-input mb-4">
              ...
              <input
                type="text"
                defaultValue={
                  updatePage ? updateBlogData?.title : (state.title as string)
                }
                name="title"
                id="title"
                onChange={() => titleError.length > 0 && setTitleError("")}
            ...
            <div className="title-input my-2 select-none relative">
              ...
              <div
                className={`w-full flex justify-between items-center my-2 p-2 md:p-3 text-lg rounded-lg ring-1 cursor-pointer hover:bg-gray-50  ${
                  !isCategoryMenuOpen ? "ring-gray-300" : "ring-blue-400"
                }`}
                onClick={() => {
                    ...
                    >
                      {
                        <>
                          ...
                          !image ? (
                            <div className="w-full relative h-64 flex items-center justify-center py-4 p-2">
                              <img
                                src={updateBlogData.featuredImage.secureUrl}
                                alt=""
                                className="h-full rounded-lg"
                              />
                            </div>
            ...
            <p className="text-red-400 text-sm h-5">
              {(content.length >= 10 && state.error?.content?._errors[0]) ?? ""}
            </p>
